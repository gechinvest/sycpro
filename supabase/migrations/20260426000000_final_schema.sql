-- Consolidated Schema for SmartYield Capital (SYC)
-- Created: 2026-04-26
...
CREATE TABLE recharges (
    ...
    ft_id TEXT NOT NULL,
    screenshot_url TEXT,
    payment_gateway TEXT CHECK (payment_gateway IN ('manual', 'telebirr', 'cbe', 'abyssinia')) DEFAULT 'manual',
...
CREATE POLICY "Users can insert their own recharges" ON recharges FOR INSERT WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY "Admins can view all bank accounts" ON bank_accounts FOR ALL USING (
    EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND is_admin = TRUE)
);

CREATE POLICY "Admins can view all profit logs" ON profit_logs FOR ALL USING (
    EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND is_admin = TRUE)
);

CREATE POLICY "Admins can view all referrals" ON referrals FOR ALL USING (
    EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND is_admin = TRUE)
);
...
DECLARE
    new_profile_id UUID;
BEGIN
    INSERT INTO public.profiles (id, full_name, phone, email, referral_code, wallet_balance, total_earnings)
    VALUES (
        new.id,
        ...
        70.00, -- Registration bonus
        70.00
    ) RETURNING id INTO new_profile_id;

    -- Log the registration bonus in profit_logs
    INSERT INTO public.profit_logs (user_id, amount, type)
    VALUES (new_profile_id, 70.00, 'registration_bonus');
